...
from tinyvit.tiny_vit_sam import TinyViT

# %%
parser = argparse.ArgumentParser()
parser.add_argument('-i', '--tr_npy_path', type=str,
                    #default='../../data/npy_litemedsam/train',
...
parser.add_argument('-tinyvit_checkpoint', type=str,
                    default='Tinyvit_pretrained.pt',
                    help='path to TinyViT encoder checkpoint (not required)')
parser.add_argument('-work_dir', type=str, default='../../synapse_result/distill_tinyvit/mse',
...
class NpyDataset(Dataset):
    ...
    def __getitem__(self, index):  # 14
        img_name = os.path.basename(self.gt_path_files[index])  # CT_Abd_FLARE22_Tr_0001-014.npy
        img_3c = np.load(join(self.img_path, img_name), 'r', allow_pickle=True)  # (1024, 1024, 3)
        ...
        resize_img_skimg_01 = (resize_img_skimg - resize_img_skimg.min()) / np.clip(
            resize_img_skimg.max() - resize_img_skimg.min(), a_min=1e-8,
            a_max=None)  # normalize to [0, 1], (1024, 1024, 3)
        # convert the shape to (3, H, W)
        img_1024 = np.transpose(resize_img_skimg_01, (2, 0, 1))  # (3,1024,1024)
...
teacher_model = deepcopy(medsam_model.image_encoder)  # 只用image_encoder
...
#     map_location="cpu"
# )
# #student_model.load_state_dict(pretrained_student_ckpt["model"])
...
for epoch in range(start_epoch, max_epochs):
    ...
    for step, (teacher_input, student_input, image_name) in enumerate(pbar):
        ...
        student_output = student_model(student_input)  # (b,256,64,64)
