...
parser.add_argument('-tinyvit_checkpoint', type=str,
                    default='../../synapse_result/distill/mse(sam)/medsam_lite_encoder_best.pth',
                    help='path to TinyViT encoder checkpoint (not required)')
parser.add_argument('-work_dir', type=str,
                    default='../../synapse_result/distill/mse(sam)+mse_0.1triplet_0.3',
...
for epoch in range(start_epoch, max_epochs):
    ...
    for step, (teacher_input, student_input, image_name) in enumerate(pbar):
        ...
        with torch.no_grad():
            # patch(=kernel_size):16*16 # 每个patch编码为768维向量 # 1024/16=64 # output_size:256(由768变为256)
        ...
        triplet_loss = Triplet_loss(mixed_query_layer, mixed_key_layer, image_name[0])
