...
parser = argparse.ArgumentParser()  # ../../result/infer/lite_TransSC_medsam/tr_mse_100epoch/
parser.add_argument('-s', '--seg_dir', default='../../synapse_result/infer/lite_TransSC_medsam/mse+mse_0.1triplet_0.01tr_0.3_100epoch/segs', type=str)
parser.add_argument('-g', '--gt_dir', default='../../data/data_synapse/testdata_npz/gts/', type=str)
parser.add_argument('-csv_dir', default='../../synapse_result/infer/lite_TransSC_medsam/mse+mse_0.1triplet_0.01tr_0.3_100epoch/metrics.csv', type=str)
...
if __name__ == '__main__':
    ...
    # Add tol_avg row to compute the average of each column
    tol_avg = df[['dsc', 'nsd']].mean().round(4) if compute_NSD else df[['dsc']].mean().round(4)
    tol_avg['case'] = 'tol_avg'
    df = pd.concat([df, pd.DataFrame([tol_avg])], ignore_index=True)
